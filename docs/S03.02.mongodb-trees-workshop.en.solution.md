...
- [Section 3 : Schema Validation](#section-3--schema-validation)
  ...
  - [Load the whole dataset](#load-the-whole-dataset)
...
In this practice we will only use on the Trees dataset. It's a real world datasets with its shares of anomalies.

## The work

You goal is to

- load the dataset into a MongoDB collection on your **local**
...
GeoJSON is a lightweight data format based on JSON for representing geographical features like points, lines, and polygons, along with related data.
...
* Works seamlessly with JavaScript-based mapping libraries (e.g., Leaflet, OpenLayers)
* Supported by many GIS tools (like QGIS, ArcGIS)
* Can be used in web APIs and RESTful services
* Easy to integrate with databases that support JSON (like PostgreSQL/PostGIS)
* Can be parsed by most programming languages that handle JSON (Python, JavaScript, etc.)

Facilitates working with polygons

🔹 Calculate area & perimeter
🔹 Check if point is inside polygon – Useful for location-based queries (e.g., is a user in a service area?).
🔹 Find intersections – Identify overlapping areas between polygons.
🔹 Clip or merge polygons – Cut polygons by boundaries or join adjacent ones.

etc

## A look at the datasets

You can find the Tree dataset in the github repo

<https://github.com/SkatAI/epita-mongodb/tree/master/data>

The dataset exists in the following versions

- 100 samples: `trees_100.json.zip`
- 1000 samples: `trees_1k.json.zip`
- full dataset in JSON: `trees.json.zip`
- full dataset in NDJSON: `trees.ndjson.zip`
...
Also note that the geo location value `geo_point_2d` is a string. We will convert it into a `Point` data type using an aggregation pipeline for geoJson queries


### Visually checking a tree with google maps

As you will see some trees have anomalies in their measurements. The max height is 2524 meters and the max circumference is 80105 cm. This is most probably a human error. (or [magic beans](https://en.wikipedia.org/wiki/Jack_and_the_Beanstalk))

For some other fields, it is less obvious if the measure is an anomaly or if the tree is really that high or that big.
...
3. Since loading the whole dataset may take awhile, **it's best to start working on a 1k or 100 sample subset**. All examples below use the 1k trees subset.
...
If you are able to load the 1k tree sample file, you should now load the entire dataset. This takes a few minutes.
...
Write a validator that requires
...
What do you observe? What's your conclusion?
