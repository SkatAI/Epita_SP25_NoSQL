- [S02.01 MongoDB: Deep Dive](#s0201-mongodb-deep-dive)
...
An excellent article on when to use NoSQL: <https://medium.com/@sqlinsix/when-to-use-sql-or-nosql-b50d4a52c157> (available as PDF in the github repo in ./reading)
...
[Atlas SQL Interface](https://www.mongodb.com/docs/atlas/data-federation/query/sql/getting-started/), [Mongo BI connector](https://www.mongodb.com/docs/bi-connector/current/what-is-the-bi-connector/)
...
There's a series of _entities_ compatible with a SQL database:
...
In a normalized SQL database, one piece of information exists in a single table.

In such a normalized SQL database, we would need to perform a significant number of joins between the `session` table and those of the different types of `exercises` and the day's workout.
...
**Note**: PostgreSQL now has [JSON data types](https://www.postgresql.org/docs/current/datatype-json.html) that can be used for such polymorphic data cases.

### Data polymorphism

When data has fields that vary significantly, a NoSQL document database is better suited for the final application.
